...
<template>
  <section>
    <HeaderComponent />
    <RouteComponent :text-routes="['Inicio', 'Productos', UnitProduct.productName]"
    ...
    <div class="flex w-[80%] gap-10 mx-auto mt-20 mb-28">
      <div class="w-[40%] flex items-center">
        <button @click="changeImage('left')" class="group">
          <Icon name="mdi-light:chevron-left" size="2rem"
        ...
        <button @click="changeImage('right')" class="group">
          <Icon name="mdi-light:chevron-right" size="2rem"
            class="text-4xl group-hover:text-orange-500 duration-150 cursor-pointer" />
        </button>
      </div>
    ...
    <div class="mt-24 font-monserrat w-[80%] mx-auto text-center">
      ...
      <div class="flex gap-2 mt-10 w-full">
        <div class="flex gap-5 mx-auto">
          <ProductComponent v-for="(item, index) in recommendedItems" :id="item.id" :key="index" :image="item.image"
            :category="item.category" :dispatch="item.dispatch" :old="item.old" :isDiscount="item.isDiscount"
            :price="item.price" :productName="item.productName" :stock="item.stock" />
        </div>
      </div>
    </div>
  </section>
  <section>
    <div
      ...
      <form @submit.prevent="1" action="." class="flex flex-col gap-4 w-full">
        ...
        <button @click="showToast('check', 'Enviado correctamente')"
