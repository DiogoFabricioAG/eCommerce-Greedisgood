<template>
  <div class="flex flex-col items-center justify-center min-h-screen transition-all duration-500 ease-in-out">
    <h1
      :class="[
    ...
    <p :class="['opacity-0 duration-300 translate-y-[-250px] absolute font-ubuntu' , {'opacity-100' : !initPage}]">Escoge lo que necesites</p>


    <div
      v-if="!initPage"
      class="mt-20 bg-white p-8 rounded-lg shadow-lg transition-all duration-500 ease-in-out opacity-0"
      :class="{ 'opacity-100 translate-y-0': !initPage, 'w-1/2' : !isLogin , 'w-1/3' : isLogin }"
    >
      <div class="flex justify-center mb-4">
        <button
          @click="isLogin = true; currentPage = 0"
          :class="['px-4 py-2', isLogin ? 'text-orange-500 font-bold' : 'text-gray-500']"
        >
          Login
        </button>
      ...
      <form @submit.prevent="handleFormSubmit" :class="['space-y-4 duration-150']">
        ...
        <div v-else class="relative w-full">
          <div
            :class="[
              ...
              currentPage === 0 ? 'opacity-100 relative' : 'opacity-0 h-0 overflow-hidden'
            ]"
          >
            <div>
              <label class="block text-gray-700">Email</label>
          ...
          <div
            :class="[
              'duration-500 transition-all ease-in-out absolute w-full',
              currentPage === 1 ? 'opacity-100 relative' : 'opacity-0 h-0 overflow-hidden'
            ]"
          >
            <div>
          ...
          <div
            :class="[
              'duration-500 transition-all ease-in-out absolute w-full',
              currentPage === 2 ? 'opacity-100 relative' : 'opacity-0 h-0 overflow-hidden'
            ]"
          >
            <div>
        ...
        <div class="flex gap-2 w-full" v-else>
          <button
            @click="currentPage--"
            type="button"
            :class="['w-full bg-orange-500 text-white py-2 rounded hover:bg-orange-600 transition', currentPage === 0 ? 'opacity-0' : 'opacity-100']">
            Pagina Anterior
          </button>
          <button
