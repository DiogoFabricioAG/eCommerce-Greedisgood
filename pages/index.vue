...
const productItems = ref<productItems[]>([
  {
    category: "Toys & Fun",
    image: "https://imagedelivery.net/4fYuQyy-r8_rpBpcY7lH_A/falabellaPE/14820959_1/w=800,h=800,fit=pad"
  },
  {
    category: "Toys & Fun",
    image: "https://imagedelivery.net/4fYuQyy-r8_rpBpcY7lH_A/falabellaPE/14820959_1/w=800,h=800,fit=pad"
  ...
  {
    category: "Shirts",
    image: "https://isto.pt/cdn/shop/files/Heavyweight_Black_ef459afb-ff7a-4f9a-b278-9e9621335444.webp?v=1710414950"
  },
  {
    category: "Shirts",
    image: "https://isto.pt/cdn/shop/files/Heavyweight_Black_ef459afb-ff7a-4f9a-b278-9e9621335444.webp?v=1710414950"
  ...
  <section>
    <div class="flex items-center min-w-[50%] h-[80vh] w-4/5 mt-7 mb-12 mx-auto gap-10 justify-center">
      ...
      <div class="flex flex-col gap-4 justify-between">
        ...
        <ul class="space-y-2">
          <ListComponent v-for="(item, index) in itemsList" :key="index" :icon_name="item.icon_name">
            {{ item.text }}
          </ListComponent>
        </ul>
      </div>
  ...
  <section>
    ...
    <div class="flex justify-center  items-center gap-5 mx-auto mt-5">
      <button @click="handleMouvement('left')">
        <Icon name="mdi-light:chevron-left" class="hover:text-orange-500 duration-100" size="4rem" />
      </button>
      
      <ProductPresentationComponent v-for="(item, index) in productItems.slice(0, 3)" :key="index" :category="item.category" :image="item.image"/>
      <button @click="handleMouvement('right')">
        <Icon name="mdi-light:chevron-right" class="hover:text-orange-500 duration-100" size="4rem" />
      </button>
    </div>
  </section>
  <section>
    <div class="my-10 w-4/5 mx-auto rounded shadow-lg items-center bg-gradient-to-r from-orange-500 to-white text-white flex justify-between p-5">
      ...
      <form @submit.prevent="1" action="." class="flex flex-col  justify-center items-end gap-3 font-sans">
        <input type="email" class="p-2 text-black rounded-md border border-gray-300 focus:outline-none" placeholder="Ingresa tu correo" />
        <button class="shadow-md bg-white text-orange-500 hover:text-orange-700 font-bold p-2 duration-100 text-sm border-black focus:scale-90" type="submit">Suscribirme</button>
      </form>
    </div>
  </section>
  <section>
